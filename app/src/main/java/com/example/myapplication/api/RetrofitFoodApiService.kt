...
import com.example.myapplication.adapters.NutritionResponseToFrontFoodAdapter
...
//    override suspend fun getDogtionaryEntriesFlow(): Flow<List<FullInformationRecipe>> = favoritesEntryFlow
    ...
    override suspend fun getRandomRecipe(): FullInformationRecipe =
        fullInformationAdapter.adapt(foodApi.getRandomRecipe(API_KEY).first())!!
...
//    override suspend fun findByNutrients( minCarbs :Int, maxCarbs:Int ,minFat:Int , maxFat:Int , minCalories:Int , maxCalories:Int ,minProtein :Int, maxProtein:Int ): List<FrontFood> =
//        foodApi.findByNutrients(API_KEY,  minCarbs,      maxCarbs,     minFat,      maxFat,      minCalories,      maxCalories,     minProtein,      maxProtein).results.mapNotNull { adapter.adapt(it!!) }
